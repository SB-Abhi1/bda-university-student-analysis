...
grouped = GROUP filtered BY grade;

grade_count = FOREACH grouped GENERATE group AS grade, COUNT(filtered) AS total_students;

result = ORDER grade_count BY total_students DESC;

STORE result INTO '/user/output_pig2' USING PigStorage(',');
