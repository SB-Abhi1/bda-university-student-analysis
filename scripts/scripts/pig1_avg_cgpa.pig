...
avg_cgpa = FOREACH grouped GENERATE group AS department, AVG(filtered.cgpa) AS avg_cgpa;

result = ORDER avg_cgpa BY avg_cgpa DESC;

STORE result INTO '/user/output_pig1' USING PigStorage(',');
